...
pub fn init_downstream_modules(edgion_http: &EdgionHttp, modules: &mut HttpModules) {
    ...
    let enable_compression = edgion_http.edgion_gateway_config.spec.server.as_ref()
    ...
    if edgion_http.enable_http2 {
        ...
        tracing::info!(gateway=%edgion_http.gateway_name, listener=%edgion_http.listener.name, "GrpcWeb module enabled");
    }
}
