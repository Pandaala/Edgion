    ...
    pub fn build(routes: Vec<Arc<HttpRouteRuleUnit>>) -> Self {
        if routes.is_empty() {
            tracing::debug!(
                ...
                "Built empty regex routes engine"
            );
            return Self {
                regex_set: None,
                routes: Vec::new(),
...
mod tests {
    use super::*;
    use crate::types::{HTTPRouteMatch, HTTPRouteRule, HTTPPathMatch};
    ...
    fn create_test_route_unit(
        ...
        let regex = Regex::new(regex_pattern)
            .expect(&format!("Invalid regex pattern: {}", regex_pattern));
    ...
    fn test_complex_regex_patterns() {
        let routes = vec![
            create_test_route_unit("default", "uuid", 
                r"^/items/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$", 0),
            create_test_route_unit("default", "date", 
                r"^/logs/\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$", 1),
            create_test_route_unit("default", "version", 
                r"^/api/v[0-9]+/.*$", 2),
        ];
