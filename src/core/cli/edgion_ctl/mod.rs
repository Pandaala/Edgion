...
impl Cli {
    pub async fn run(&self) -> Result<()> {
        ...
        match &self.command {
            ...
            Commands::Get { kind, name, namespace, output } => {
                let format = OutputFormat::from_str(output)?;
                commands::get::get(&client, kind, name.as_deref(), namespace.as_deref(), format).await
            }
            Commands::Delete { kind, name, namespace, file } => {
                commands::delete::delete(&client, kind.as_deref(), name.as_deref(), namespace.as_deref(), file.as_deref()).await
            }
            Commands::Reload => {
