...
fn create_server_conf(config: &EdgionGatewayConfig) -> ServerConf {
    ...
    conf.threads = config.server.threads
        .unwrap_or_else(|| {
    ...
    conf.grace_period_seconds = Some(
        config.server.grace_period_seconds.unwrap_or(30)
    );
    
    // 4. Graceful shutdown timeout (default: 10 seconds)
    conf.graceful_shutdown_timeout_seconds = Some(
        config.server.graceful_shutdown_timeout_seconds.unwrap_or(10)
    );
    
    // 5. Upstream keepalive pool size (default: 128)
    conf.upstream_keepalive_pool_size = config.server.upstream_keepalive_pool_size
        .unwrap_or(128);
