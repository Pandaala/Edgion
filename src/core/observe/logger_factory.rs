...
use crate::core::observe::ssl_log::SslLogger;
use crate::types::{LogConfig, LocalFileWriterConfig, StringOutput};

/// Create an async logger (for Access, TCP, UDP logs)
/// Returns None if the log is disabled
pub async fn create_async_logger(
    config: &LogConfig,
    log_type: &str,
) -> Result<Option<Arc<AccessLogger>>> {
    if !config.enabled {
...
/// Returns None if the log is disabled
pub async fn create_sync_logger(
    config: &LogConfig,
) -> Result<Option<SslLogger>> {
    if !config.enabled {
    ...
    match &config.output {
        StringOutput::LocalFile(file_cfg) => {
            ...
            let writer = LocalFileWriter::new(writer_config)
                .with_log_type(LogType::Ssl);
