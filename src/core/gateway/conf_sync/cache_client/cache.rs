...
impl<T: ResourceMeta + Resource> ClientCache<T> {
    ...
    pub fn set_conf_processor(&self, processor: Box<dyn crate::core::common::conf_sync::traits::ConfHandler<T> + Send + Sync>)
    where
    ...
    pub fn trigger_update_event_by_key(&self, key: &str) {
        ...
        cache.add_compress_event(
            key.to_string(),
            crate::core::common::conf_sync::traits::ResourceChange::EventUpdate,
        );
