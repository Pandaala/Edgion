...
impl LeaderElection {
    ...
    pub async fn run(&self, mut shutdown_signal: Option<super::ShutdownSignal>) -> Result<()> {
...
impl LeaderHandle {
    ...
    pub async fn wait_until_leader_with_shutdown(&self, mut shutdown: super::ShutdownSignal) -> bool {
        loop {
            if self.is_leader() {
                return true;
            }
