...
/// Generate a short connection ID for TLS log correlation.
/// Format: 12 lowercase hex chars from UUID v4 (e.g. "7f1a3c9b2e4d").
/// 48 bits of randomness — collision probability is negligible.
pub fn gen_tls_id() -> String {
    let id = uuid::Uuid::new_v4().simple().to_string();
    id[..12].to_string()
}

/// Describes which certificate source was matched during TLS handshake.
#[derive(Debug, Clone)]
pub enum CertSource {
    /// Certificate from EdgionTls CRD
    EdgionTls { namespace: String, name: String },
    /// Certificate from Gateway Listener TLS config
    GatewayTls {
        gateway_namespace: String,
        gateway_name: String,
        listener_name: String,
    },
    /// No certificate found (SNI didn't match anything)
    NotFound,
}
...
pub struct TlsConnMeta {
    ...
    /// Short hex string generated by gen_tls_id().
    pub tls_id: String,
    ...
    /// Which certificate was matched and applied during handshake.
