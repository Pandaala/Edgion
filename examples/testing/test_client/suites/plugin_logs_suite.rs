                ...
                match request.send().await {
                    Ok(response) => {
                        ...
                        if let Some(debug_header) = response.headers().get("x-debug-access-log") {
                                        ...
                                        Ok(access_log) => {
                                            ...
                                            if access_log.plugin_logs.is_empty() {
                                                ...
                                                TestResult::passed_with_message(
                                                    start.elapsed(),
                                                    format!("Array structure verified, {} stages found", access_log.plugin_logs.len())
                                                )
                                            } else {
                                                TestResult::failed(
                                                    start.elapsed(),
                                                    format!("Expected 2 stages, found {}", access_log.plugin_logs.len())
                                                )
                                            }
                                        }
                            ...
                            TestResult::failed(
                                start.elapsed(),
                                "X-Debug-Access-Log header not found".to_string()
                            )
                        }
                    }
                        ...
                        if let Some(debug_header) = response.headers().get("x-debug-access-log") {
                            if let Ok(json_str) = debug_header.to_str() {
                                if let Ok(access_log) = serde_json::from_str::<AccessLog>(json_str) {
                                    ...
                                    if let Some(stage) = access_log.plugin_logs.iter().find(|s| s.stage == "request_filters") {
                                        // Check if it has 2 plugins
                                        if stage.logs.len() != 2 {
                                            return TestResult::failed(
                                                start.elapsed(),
                                                format!("Expected 2 plugins in request_filters, found {}", stage.logs.len())
                                            );
                                        }
                                        
                                        // Check plugin names
                                        let plugin_names: Vec<&str> = stage.logs.iter().map(|p| p.name.as_str()).collect();
                                        if !plugin_names.contains(&"cors") || !plugin_names.contains(&"csrf") {
                                            return TestResult::failed(
                                                start.elapsed(),
                                                format!("Expected 'cors' and 'csrf' plugins, found: {:?}", plugin_names)
                                            );
                                        }
                        ...
                        if let Some(debug_header) = response.headers().get("x-debug-access-log") {
                            if let Ok(json_str) = debug_header.to_str() {
                                if let Ok(access_log) = serde_json::from_str::<AccessLog>(json_str) {
                                    ...
                                    if let Some(stage) = access_log.plugin_logs.iter().find(|s| s.stage == "upstream_response_filters") {
                                        ...
                                        if stage.logs[0].name != "ResponseHeaderModifier" {
                                            return TestResult::failed(
                                                start.elapsed(),
                                                format!("Expected first plugin to be ResponseHeaderModifier, got {}", stage.logs[0].name)
                                            );
                                        }
                                        ...
                                        if stage.logs[1].name != "DebugAccessLogToHeader" {
                                            return TestResult::failed(
                                                start.elapsed(),
                                                format!("Expected second plugin to be DebugAccessLogToHeader, got {}", stage.logs[1].name)
                                            );
                                        }
                            ...
                            if let Ok(json_str) = debug_header.to_str() {
                                if let Ok(access_log) = serde_json::from_str::<AccessLog>(json_str) {
                                    if access_log.plugin_logs.len() != 2 {
                                        return TestResult::failed(
                                            start.elapsed(),
                                            format!("Expected exactly 2 stages, got {}", access_log.plugin_logs.len())
                                        );
                                    }
                                    ...
                                    if access_log.plugin_logs[0].stage != "request_filters" {
                                        return TestResult::failed(
                                            start.elapsed(),
                                            format!("First stage should be request_filters, got {}", access_log.plugin_logs[0].stage)
                                        );
                                    }
                                    ...
                                    if access_log.plugin_logs[1].stage != "upstream_response_filters" {
                                        return TestResult::failed(
                                            start.elapsed(),
                                            format!("Second stage should be upstream_response_filters, got {}", access_log.plugin_logs[1].stage)
                                        );
                                    }
                            ...
                            if let Ok(json_str) = debug_header.to_str() {
                                if let Ok(access_log) = serde_json::from_str::<AccessLog>(json_str) {
                                    if access_log.plugin_logs.is_empty() {
                                        ...
                                        TestResult::failed(
                                            start.elapsed(),
                                            format!("Expected empty plugin_logs, got {} stages", access_log.plugin_logs.len())
                                        )
                                    }
                                } else {
                                    TestResult::failed(start.elapsed(), "Failed to parse JSON".to_string())
                        ...
                        if let Some(debug_header) = response.headers().get("x-debug-access-log") {
                            if let Ok(json_str) = debug_header.to_str() {
                                if let Ok(access_log) = serde_json::from_str::<AccessLog>(json_str) {
                                    ...
                                    for stage in &access_log.plugin_logs {
                                        for plugin in &stage.logs {
                                            match plugin.time_cost {
                                                None => {
                                                    all_valid = false;
                                                    error_msg = format!("Plugin '{}' in stage '{}' missing time_cost", plugin.name, stage.stage);
                                                    break;
                                                }
                                                Some(tc) => {
                                                    if tc == 0 {
                                                        all_valid = false;
                                                        error_msg = format!("Plugin '{}' has time_cost = 0", plugin.name);
                                                        break;
                                                    }
                                                    // Check reasonable range (< 1 second = 1,000,000 microseconds)
                                                    if tc > 1_000_000 {
                                                        all_valid = false;
                                                        error_msg = format!("Plugin '{}' has unreasonable time_cost: {} us", plugin.name, tc);
                                                        break;
                                                    }
                                                }
                                            }
                                    ...
                                    if all_valid {
                                        let total_plugins: usize = access_log.plugin_logs.iter().map(|s| s.logs.len()).sum();
                                        TestResult::passed_with_message(
                                            start.elapsed(),
