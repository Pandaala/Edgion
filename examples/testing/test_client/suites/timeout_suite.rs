                        ...
                        if let Some(debug_header) = headers.get("X-Debug-Access-Log") {
                            if let Ok(debug_str) = debug_header.to_str() {
                                if let Ok(debug_json) = serde_json::from_str::<serde_json::Value>(debug_str) {
                                    let internal_status = debug_json["request_info"]["status"].as_u64().unwrap_or(0);
                                    if internal_status == 200 {
                                        TestResult::passed_with_message(
                                            start.elapsed(),
                                            format!("Normal response: HTTP {} / Internal {}", status.as_u16(), internal_status)
                                        )
                                    } else {
                                        TestResult::failed(
                                            start.elapsed(),
                                            format!("Status mismatch: HTTP {} but Internal {}", status.as_u16(), internal_status)
                                        )
                                    }
                                } else {
                                    TestResult::passed_with_message(
    ...
    fn test_backend_request_timeout() -> TestCase {
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                let mut request = ctx.http_client
                    .get(format!("{}/delay/5", ctx.http_url()));
                request = request.header("Host", "timeout-backend.example.com");
                            ...
                            if let Ok(debug_str) = debug_header.to_str() {
                                if let Ok(debug_json) = serde_json::from_str::<serde_json::Value>(debug_str) {
                                    ...
                                    if internal_status == 504 {
                                        TestResult::passed_with_message(
                                            start.elapsed(),
                                            format!("Backend request timeout: HTTP 504 / Internal 504 ✓")
                                        )
                                    } else {
                                        TestResult::failed(
                                            start.elapsed(),
    ...
    fn test_backend_per_try_timeout() -> TestCase {
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                let mut request = ctx.http_client
                    .get(format!("{}/delay/3", ctx.http_url()));
                request = request.header("Host", "timeout-backend.example.com");
                            ...
                            if let Ok(debug_str) = debug_header.to_str() {
                                if let Ok(debug_json) = serde_json::from_str::<serde_json::Value>(debug_str) {
                                    ...
                                    if internal_status == 504 {
                                        TestResult::passed_with_message(
                                            start.elapsed(),
                                            format!("Backend per-try timeout: HTTP 504 / Internal 504 ✓")
                                        )
                                    } else {
                                        TestResult::failed(
                                            start.elapsed(),
    ...
    fn test_client_read_timeout_499() -> TestCase {
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                // 但客户端3秒后会主动断开
                let mut request = short_timeout_client
                    .get(format!("{}/delay/10", ctx.http_url()));
                ...
                match request.send().await {
                    Ok(response) => {
                        ...
                        TestResult::failed(
                            elapsed,
                            format!("Expected client to timeout but got HTTP {} after {}s", status.as_u16(), elapsed.as_secs())
                        )
                    }
                    Err(e) => {
                        let elapsed = start.elapsed();
                        ...
                        if is_timeout || elapsed.as_secs() >= 2 && elapsed.as_secs() <= 5 {
                            ...
                            TestResult::failed(
                                elapsed,
                                format!("Unexpected error or timing: {}s, error: {}", elapsed.as_secs(), error_msg)
                            )
                        }
                    }
                }
    ...
    fn test_client_read_timeout() -> TestCase {
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                let mut request = long_timeout_client
                    .get(format!("{}/delay/20", ctx.http_url()));
                request = request.header("Host", "timeout-client.example.com");
                ...
                match request.send().await {
                    Ok(response) => {
                        ...
                        if status_code == 200 {
                            TestResult::passed_with_message(
                                elapsed,
                                format!("Got 200 OK after {}s - client/backend timeouts not triggered (both > 20s)", elapsed.as_secs())
                            )
                        } else {
                            ...
                            if let Some(debug_header) = headers.get("X-Debug-Access-Log") {
                                if let Ok(debug_str) = debug_header.to_str() {
                                    if let Ok(debug_json) = serde_json::from_str::<serde_json::Value>(debug_str) {
                                        ...
                                        TestResult::failed(
                                            elapsed,
                                            format!("Expected 200 but got HTTP {} / Internal {}", status_code, internal_status)
                                        )
                                    } else {
                                        TestResult::failed(elapsed, format!("Got HTTP {} (debug JSON parse error)", status_code))
                                    }
                                } else {
