        ...
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                match request.send().await {
                    ...
                    Err(e) => TestResult::failed(
                        start.elapsed(),
                        format!("Backend TLS request failed: {}", e)
                    ),
                }
            })
        )
        ...
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                match request.send().await {
                        ...
                        match response.text().await {
                            Ok(body) => {
                                ...
                                if status.is_success() 
                                    ...
                                    TestResult::passed_with_message(
                                        start.elapsed(),
                                        format!("Request forwarded successfully via Backend TLS")
                                    )
                                } else {
                                    TestResult::failed(
                                        start.elapsed(),
                    ...
                    Err(e) => TestResult::failed(
                        start.elapsed(),
                        format!("Backend TLS echo request failed: {}", e)
                    ),
                }
            })
        )
        ...
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                match request.send().await {
                        ...
                        match response.text().await {
                            Ok(body) => {
                                ...
                                if status.is_success() && body.contains("backend-tls-trace-123") {
                                    TestResult::passed_with_message(
                                        start.elapsed(),
                                        format!("Headers forwarded correctly through Backend TLS")
                                    )
                                } else {
                                    TestResult::failed(
                                        start.elapsed(),
                    ...
                    Err(e) => TestResult::failed(
                        start.elapsed(),
                        format!("Backend TLS headers request failed: {}", e)
                    ),
                }
            })
        )
