        ...
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                let mut request = client.get(&url)
                    .header("x-forwarded-for", normal_xff);
    ...
    fn test_excessive_xff() -> TestCase {
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                assert!(long_xff.len() > 200, "Test XFF should be > 200 bytes, got {}", long_xff.len());
                
                let mut request = client.get(&url)
                    .header("x-forwarded-for", &long_xff);
                ...
                match request.send().await {
                    Ok(response) => {
                        if response.status().is_success() {
                            TestResult::passed_with_message(
                                start.elapsed(),
                                format!("✓ Boundary XFF accepted (exactly 200 bytes)")
                            )
                        } else {
                            TestResult::failed(
                                start.elapsed(),
