    ...
    fn test_xff_extraction() -> TestCase {
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                headers.insert("x-forwarded-for", "203.0.113.1, 198.51.100.2, 192.168.1.1".parse().unwrap());
                    ...
                    Ok(resp) => {
                        ...
                        match resp.json::<Value>().await {
                            Ok(body) => {
                                ...
                                // Verify X-Forwarded-For has client_addr (127.0.0.1) appended
                                ...
                                let expected_xff = "203.0.113.1, 198.51.100.2, 192.168.1.1, 127.0.0.1";
                                if xff != expected_xff {
                                    return TestResult::failed(
                                        start.elapsed(),
                                        format!("Expected XFF='{}', got '{}'", expected_xff, xff)
                                    );
                                }
                    ...
                    Ok(resp) => {
                        ...
                        match resp.json::<Value>().await {
                            Ok(body) => {
                                ...
                                // Without XFF, X-Real-IP should be client_addr (127.0.0.1)
                                let real_ip = match headers.get("x-real-ip").and_then(|v| v.as_str()) {
                                    Some(ip) => ip,
                                    None => return TestResult::failed(
                                        start.elapsed(),
                                ...
                                if real_ip != "127.0.0.1" {
                                    return TestResult::failed(
                                        start.elapsed(),
                                        format!("Expected X-Real-IP=127.0.0.1 (direct connection), got {}", real_ip)
                                    );
                                }
                                ...
                                if xff != "127.0.0.1" {
                                    return TestResult::failed(
                                        start.elapsed(),
                                        format!("Expected XFF='127.0.0.1', got '{}'", xff)
                                    );
                                }
