                ...
                match request.send().await {
                    Ok(response) => {
                        if response.status() == 404 {
                            TestResult::passed_with_message(
                                start.elapsed(),
                                "Header exact match works correctly (matched correct value, rejected wrong value)".to_string()
                            )
                        } else {
                            TestResult::failed(
                                start.elapsed(),
    ...
    fn test_query_param_match() -> TestCase {
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                // 正面测试：两个查询参数都匹配
                let mut request = ctx.http_client.get(format!("{}/query-test?apikey=secret123&version=10", ctx.http_url()));
                request = request.header("Host", "match-test.example.com");
                ...
                let mut request = ctx.http_client.get(format!("{}/query-test?apikey=secret123&version=abc", ctx.http_url()));
                request = request.header("Host", "match-test.example.com");
                ...
                match request.send().await {
                    Ok(response) => {
                        if response.status() == 404 {
                            TestResult::passed_with_message(
                                start.elapsed(),
                                "Query param match works correctly (matched valid params, rejected invalid)".to_string()
                            )
                        } else {
                            TestResult::failed(
                                start.elapsed(),
