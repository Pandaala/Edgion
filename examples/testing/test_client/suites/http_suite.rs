    ...
    fn test_status_codes() -> TestCase {
        TestCase::new(
            ...
            |ctx: TestContext| Box::pin(async move {
                ...
                TestResult::passed_with_message(
                    start.elapsed(),
                    "All status codes returned correctly".to_string()
                )
            })
        )
    }
                    ...
                    .await
                {
                    Ok(response) => {
                        ...
                        if response.status().is_success() && elapsed.as_secs() >= delay_seconds {
                            TestResult::passed_with_message(
                                elapsed,
                                format!("Delayed {}s as expected", delay_seconds)
                            )
                        } else {
                            TestResult::failed(
                                elapsed,
                                format!("Delay not working correctly. Elapsed: {:?}", elapsed)
                            )
                        }
                    }
                    Err(e) => TestResult::failed(start.elapsed(), e.to_string()),
