...
// - 01_EdgionStreamPlugins_deny.yaml            # deny-all resource (plugins field)
// - 02_EdgionStreamPlugins_allow_localhost.yaml  # allow 127.0.0.0/8 only (plugins field)
// - 06_EdgionStreamPlugins_tls_route_deny.yaml   # deny-all via tlsRoutePlugins (Stage 2)
// - 03_Gateway.yaml                              # Gateway with 3 TLS listeners
// - 04_TLSRoute_allow_localhost.yaml             # Route with allow-localhost plugin
// - 05_TLSRoute_deny.yaml                        # Route with deny-all plugin
// - 07_TLSRoute_tls_route_plugins_deny.yaml      # Route using tlsRoutePlugins
// - Service_test-tcp.yaml / EndpointSlice_test-tcp.yaml
//
// Port allocation (from ports.json "TLSRoute/StreamPlugins"):
// - 31282 (tls): allow-localhost filter (backward compat: plugins field)
// - 31283 (tls_filtered): deny-all filter (backward compat: plugins field)
// - 31287 (tls_route_plugins): deny-all via tlsRoutePlugins (Stage 2)
...
impl TlsStreamPluginsTestSuite {
    ...
    /// Test: tlsRoutePlugins field (Stage 2) should deny connections
    fn test_tls_route_plugins_deny() -> TestCase {
        TestCase::new(
            "tls_route_plugins_deny",
            "tlsRoutePlugins (Stage 2) IpRestriction should deny connection after TLS handshake + route match",
            |ctx: TestContext| {
                Box::pin(async move {
                    let start = Instant::now();
                    let addr = format!("{}:31287", ctx.target_host);
                    let connector = make_tls_connector();
                    let sni = ServerName::try_from("test.sp-route-deny.example.com").unwrap();
                        ...
                        Ok(Ok(tcp_stream)) => match connector.connect(sni, tcp_stream).await {
                            Ok(mut tls_stream) => {
                                ...
                                match tokio::time::timeout(Duration::from_secs(2), tls_stream.read(&mut buf)).await {
                                    Ok(Ok(0)) => TestResult::passed_with_message(
                                        start.elapsed(),
                                        "Connection closed by TLS route plugin (Stage 2)".to_string(),
                                    ),
                                    Ok(Ok(n)) => TestResult::failed(
                                        start.elapsed(),
                                        format!("Expected Stage 2 denial but received {} bytes", n),
                                    ),
                                    Ok(Err(_e)) => TestResult::passed_with_message(
                                        start.elapsed(),
                                        "Connection rejected by TLS route plugin (Stage 2)".to_string(),
                                    ),
                                    Err(_) => TestResult::failed(
                                        start.elapsed(),
                                        "Read timed out — TLS route plugin did not deny".to_string(),
                                    ),
                                }
                            }
                            Err(_e) => TestResult::passed_with_message(
...
impl TestSuite for TlsStreamPluginsTestSuite {
    ...
    fn test_cases(&self) -> Vec<TestCase> {
        vec![
            ...
            Self::test_deny_all(),
            Self::test_tls_route_plugins_deny(),
