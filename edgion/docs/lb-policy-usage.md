...
rules:
  - filters:
      - type: ExtensionRef
        extensionRef:
          name: ketama,fnvhash,leastconn  # 多个算法用逗号分隔
    backendRefs:
      - name: api-service
        port: 8080
```
...
```yaml
algorithms: ketama,fnvhash
algorithms: leastconn
algorithms: ketama,leastconn
```

## 工作原理

1. **策略提取**: 当 HTTPRoute 被创建或更新时，Edgion 会扫描所有 `ExtensionRef` 类型的 filter
2. **算法解析**: 从 `extensionRef.name` 中解析算法配置（逗号分隔）
3. **服务映射**: 将解析的算法应用到该 rule 的所有 backendRefs 指定的服务
...
- **策略合并**: 如果多个 HTTPRoute 为同一服务配置了不同的算法，策略会合并（去重）
...
spec:
  ...
  rules:
    - filters:
        - type: ExtensionRef
          extensionRef:
            name: ketama,fnvhash,leastconn  # 启用多个算法
      backendRefs:
        - name: multi-algo-service
          port: 8080
```

## 注意事项

1. **算法格式**: `extensionRef.name` 中的算法名称不区分大小写，多个算法用逗号分隔，不要有空格
2. **作用范围**: 算法配置会应用到同一个 rule 中所有的 backendRefs
3. **策略合并**: 如果多个 HTTPRoute 为同一个服务配置了不同的算法，策略会合并（去重）
4. **默认行为**: 未配置策略的服务继续使用默认的 RoundRobin 算法
5. **性能**: 可选算法只在需要时初始化，未使用的算法仅占用 24 字节内存
...
- **格式错误**: 多个算法用逗号分隔，不要有空格，例如 `ketama,fnvhash` 而不是 `ketama, fnvhash`
