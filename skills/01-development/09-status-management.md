...
**`Accepted: False` MUST cascade to `Programmed: False` + `Ready: False`.**

If a resource is not accepted, it cannot be programmed or ready. All handlers
must explicitly set Programmed and Ready to False when Accepted is False.
Never leave stale True conditions from a previous valid state.

## 3. Resource Categories and Status Patterns

### 3.1 Simple resources (flat status with conditions)
...
fn update_status(&self, obj: &mut T, _ctx: &HandlerContext, validation_errors: &[String]) {
    ...
    if validation_errors.is_empty() {
        ...
        update_condition(&mut status.conditions, condition_false(
            READY, INVALID, "Resource has configuration errors", generation,
        ));
    }
...
### 3.2 Per-parent resources (per-parent status)
...
fn update_status(&self, resource: &mut T, _ctx: &HandlerContext, validation_errors: &[String]) {
    ...
    if let Some(parent_refs) = &resource.spec.parent_refs {
        for parent_ref in parent_refs {
            ...
            // Set all four standard conditions
...
### 3.3 Gateway (hybrid: gateway-level + per-listener)

Gateway has both gateway-level conditions (Accepted, Programmed, Ready,
ListenersNotValid) and per-listener conditions (Accepted, ResolvedRefs,
Conflicted, Programmed, Ready). All conditions respect `validation_errors`.

### 3.4 EdgionAcme (lifecycle status)

Uses `phase`, `last_failure_reason`, etc. No standard conditions.

### 3.5 Resources without status
...
## 4. The set_parent_conditions_full Contract

```rust
pub fn set_parent_conditions_full(
    conditions: &mut Vec<Condition>,
    accepted_errors: &[AcceptedError],          // per-parent: controls Accepted/Programmed/Ready
    resolved_refs_errors: &[ResolvedRefsError],  // resource-level: controls ResolvedRefs
    observed_generation: Option<i64>,
)
```

- If `accepted_errors` is empty → Accepted: True, Programmed: True, Ready: True
- If `accepted_errors` is non-empty → Accepted: False, Programmed: False, Ready: False
  (uses first error's reason for Accepted/Programmed, "Invalid" for Ready)
- `resolved_refs_errors` → ResolvedRefs: True/False (independent of Accepted)

All four conditions are always explicitly set, preventing stale values.
...
| `ready_condition()` | Message: "Resource is ready" |
...
hasn't actually changed). Since `set_parent_conditions_full` always sets all
four standard conditions, no stale condition values survive re-processing.

## 8. Review Checklist for update_status Implementations
...
- [ ] **Accepted: False → Programmed: False + Ready: False**: never leave stale True conditions
...
### 2026-03-17: Programmed/Ready not cascaded
`set_parent_conditions_full` now explicitly sets Programmed/Ready to False
when Accepted is False. Simple resource handlers (EdgionGatewayConfig,
LinkSys, EdgionStreamPlugins) also fixed. BackendTLSPolicy and Gateway
handler fixed to cascade Programmed/Ready = False on errors.
